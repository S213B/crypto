...
def key_schedule_core(key, i):
    rotate(key)
    for idx in range(4):
        key[idx] = sbox[key[idx]]
    key[0] ^= rcon[i]

def xor_word(orig, xor_key):
    for i in range(4):
        orig[i] ^= xor_key[i]

def xor_4word(orig, xor_4key):
    xor_word(orig, xor_4key[0:4])
    xor_word(orig, xor_4key[4:8])
    xor_word(orig, xor_4key[8:12])
    xor_word(orig, xor_4key[12:16])

def key_expansion(key):
    for i in range(1, 11):
        t = key[i-1][-4:]
        key_schedule_core(t, i)
        xor_word(t, key[i-1][0:4])
        t_key = list(t)
        for j in range(1, 4):
            xor_word(t, key[-1][4*j:4*j+4])
            t_key += t
        key += [t_key]

def sub_bytes(state, enc = True):
    for i in range(4):
        for j in range(4):
            ch = state[i][j]
            if enc:
                ...
                state[i][j] = inv_sbox[ch]

def shift_rows(state, enc = True):
    for i in range(1, 4):
        if enc:
            for j in range(i):
                rotate(state[i])
...
def mix_columns(state, enc = True):
    for i in range(4):
        s0 = state[0][i]
        s1 = state[1][i]
        s2 = state[2][i]
...
def add_round_key(state, round_key):
    round_key_matrix = list_to_matrix(round_key)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key_matrix[i][j]

def list_to_matrix(text):
    return [[text[row+col*4] for col in range(4)] for row in range(4)]
...
def aes_128_encrypt(plaintext, key):
    key = [key]
    key_expansion(key)
    state = list_to_matrix(plaintext)
...
def aes_128_decrypt(ciphertext, key):
    key = [key]
    key_expansion(key)
    state = list_to_matrix(ciphertext)
    add_round_key(state, key[10])
